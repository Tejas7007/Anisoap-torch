...
/// Batched version: compute moments for N pairs at once, in parallel via Rayon.
pub fn compute_moments_batch_rust(
    ...
    use rayon::prelude::*;
    let md = max_deg as usize;
    let moment_size = (md + 1) * (md + 1) * (md + 1);

    // Each pair computes independently, then we flatten
    let results: Vec<Vec<f64>> = (0..n_pairs).into_par_iter().map(|pair| {
        let mat_offset = pair * 9;
        let cen_offset = pair * 3;
        let mut moments = vec![0.0f64; moment_size];

        let a = dil_mats[mat_offset + 0];
        let b = dil_mats[mat_offset + 1];
        let c = dil_mats[mat_offset + 2];
        let d = dil_mats[mat_offset + 4];
        let e = dil_mats[mat_offset + 5];
        let f = dil_mats[mat_offset + 8];

        let a0 = gau_cens[cen_offset + 0];
        let a1 = gau_cens[cen_offset + 1];
        let a2 = gau_cens[cen_offset + 2];

        let (cof00, cof01, cof02) = (d * f - e * e, c * e - b * f, b * e - c * d);
        let det = a * cof00 + b * cof01 + c * cof02;

        if det.abs() < 1e-14 {
            return moments; // return zeros for singular matrix
        }

        let (cov00, cov01, cov02, cov11, cov12, cov22) = (
            cof00 / det, cof01 / det, cof02 / det,
            (a * f - c * c) / det, (b * c - a * e) / det, (a * d - b * b) / det,
        );

        let global_factor = (8.0 * std::f64::consts::PI.powi(3) / det).sqrt();

        let idx = |n0: usize, n1: usize, n2: usize| -> usize {
            n0 * (md + 1) * (md + 1) + n1 * (md + 1) + n2
        };

        moments[idx(0, 0, 0)] = 1.0;
        moments[idx(1, 0, 0)] = a0;
        moments[idx(0, 1, 0)] = a1;
        moments[idx(0, 0, 1)] = a2;

        if md > 1 {
            moments[idx(2, 0, 0)] = cov00 + a0 * a0;
            moments[idx(0, 2, 0)] = cov11 + a1 * a1;
            moments[idx(0, 0, 2)] = cov22 + a2 * a2;
            moments[idx(1, 1, 0)] = cov01 + a0 * a1;
            moments[idx(0, 1, 1)] = cov12 + a1 * a2;
            moments[idx(1, 0, 1)] = cov02 + a0 * a2;
        }
        ...
        if md > 2 {
            for deg in 2..md {
                for n0 in 0..=deg {
                    for n1 in 0..=(deg - n0) {
                        let n2 = deg - n0 - n1;
                        let mom_cur = moments[idx(n0, n1, n2)];

                        let x_iter_add = if n0 > 0 { cov00 * n0 as f64 * moments[idx(n0 - 1, n1, n2)] } else { 0.0 }
                            + if n1 > 0 { cov01 * n1 as f64 * moments[idx(n0, n1 - 1, n2)] } else { 0.0 }
                            + if n2 > 0 { cov02 * n2 as f64 * moments[idx(n0, n1, n2 - 1)] } else { 0.0 };

                        moments[idx(n0 + 1, n1, n2)] = a0 * mom_cur + x_iter_add;

                        if n0 == 0 {
                            let y_iter_add = if n1 > 0 { cov11 * n1 as f64 * moments[idx(0, n1 - 1, n2)] } else { 0.0 }
                                + if n2 > 0 { cov12 * n2 as f64 * moments[idx(0, n1, n2 - 1)] } else { 0.0 };
                            moments[idx(0, n1 + 1, n2)] = a1 * mom_cur + y_iter_add;

                            if n1 == 0 {
                                moments[idx(0, 0, n2 + 1)] = a2 * mom_cur
                                    + if n2 > 0 { cov22 * n2 as f64 * moments[idx(0, 0, n2 - 1)] } else { 0.0 };
                            }
                        }
                    }
                }
    ...
    let mut all_moments = Vec::with_capacity(n_pairs * moment_size);
    for m in results {
