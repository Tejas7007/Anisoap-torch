...
# Modified to batch pair processing, Rust FFI, contractions, and aggregation.

import warnings
import metatensor
import numpy as np
import torch
from anisoap_rust_lib import compute_moments_batch
    ...
    for center_types in types:
        for neighbor_types in types:
            if (center_types, neighbor_types) in neighbor_list.keys:
                ...
                # Compute Gaussian parameters for the full block.
                ...
                # Evaluate moments in one Rust call for the full block.
                D = maxdeg + 1
                if rust_moments:
                ...
                # Contract each angular channel in batch.
                for l in range(lmax + 1):
                    deg = l + 2 * (num_ns[l] - 1)
...
def contract_pairwise_feat(pair_ellip_feat, types, show_progress=False, device=None):
    ...
    for key in ellip_keys:
        ...
        for ele in types:
            ...
            blockidx = [int(index) for index in pair_ellip_feat.keys.select(selection)]
            sel_blocks = [
